...
最终发现是逻辑上的bug，导致未收到EPOLLOUT事件就调用了onWritable函数，此时并不可写但是却调用了write函数，于是系统会返回EAGAIN，而因为高并发下大量write是无效的，就导致cpu超载和卡死。

题外话：正确的EPOLLOUT监听时机是，一，write返回-1，然后errno为EAGAIN时；二，write的返回值大于0，但小于要邱写入的字节数，即write没有完全把数据写进发送队列。
