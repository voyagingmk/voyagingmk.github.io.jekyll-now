...
\\[ \\mathbf \{ v \} = v\\mathbf \{ \\hat \{v\} \} \ \  v = |\\mathbf \{v\}|,|\\mathbf \{ \\hat \{v\} \}|=1\\]
...
\\[ p' = qp = [s,\\lambda \\mathbf \{\\hat \{v\}\}] [0,\\mathbf \{p\}] \\]
\\[ =  [-\\lambda \\mathbf \{\\hat \{v\}\}\\cdot \\mathbf \{p\}, s\\mathbf \{p\} + \\lambda \\mathbf \{\\hat \{v\}\}\\times \\mathbf \{p\} ] \\]

我们可以看到结果是一个同时有系数、有虚向量的四元数。

让我们先考虑特殊的情形：\\( \\mathbf \{p\} \\)与\\( \\mathbf \{\\hat \{v\}\} \\)正交。这种情况下，点乘部分等于0：\\(-\\lambda \\mathbf \{\\hat \{v\}\}\\cdot \\mathbf \{p\} = 0\\)。所以上面的四元数就变成了纯四元数：

\\[ p' =  [0, s\\mathbf \{p\} + \\lambda \\mathbf \{\\hat \{v\}\}\\times \\mathbf \{p\}] \\]

这时候，要使\\( \\mathbf \{p\} \\)绕\\( \\mathbf \{\\hat \{v\}\} \\)旋转，我们只需要代入\\(s=cos\\theta \\)和\\( \\lambda =sin\\theta \\)：
...
\\[ \\hat \{v\} = \\frac \{\\sqrt \{2\}\}\{2\}\\mathbf \{i\} + \\frac \{\\sqrt \{2\}\}\{2\}\\mathbf \{k\} \\]
...
但这个公式需要提供角度\\(\\theta \\)，我们可以计算\\(q\_\{1\}\\)和\\(q\_\{2\}\\)的点积从而得出角度\\(\\theta \\)：
