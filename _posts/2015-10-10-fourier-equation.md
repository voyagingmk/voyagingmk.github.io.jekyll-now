...
![5.png](../images/2015.10/5.png)

上面这个式子是说，我们随便取了一个k，并对傅里叶级数等式两边乘以\\( e\^\{-2\\pi ikt \} \\)，又因为:

\\[ e\^\{-2\\pi ikt \}c\_\{k\}e\^\{2\\pi ikt \} =c\_\{k\}e\^\{-2\\pi ikt + 2\\pi ikt \} = c\_\{k\}e\^\{0\} = c\_\{k\} \\]

所以可以把\\(c\_\{k\}\\)移到等式左边，左边的那个东东移到右边，就得到了一个\\(c\_\{k\}\\)的表达式:

![6.png](../images/2015.10/6.png)

然后再放大招：对等式两边同时积分，范围是0到1（f(t)的t的周期为1）。

积分的时候，会发现有这么个神奇的结果:

![7.png](../images/2015.10/7.png)

也就是红圈里的那个东东的积分结果是0：

![8.jpg](../images/2015.10/8.jpg)
...
（注意，\\( c\_\{-k\} = \\overline \{c\_\{k\}\} \\)哦，证明前面小节的推导是对的。）
