...
既然发送加密的密码不妥，那就是用DH密钥交换了，就不会有密钥在网络中传输的问题。当然对于用户输入密码登录的场景，DH应该是用不了了。
...
（即使是加密通讯的建立过程本身，都有完整性安全漏洞，包括下一节会说到的**算法选取**。根源在于一开始使用的非对称加密信道是单向安全而不是双向安全的。）
...
这个问题很有意思，因为我们的加密通讯已经建立了，认证机制也有了。**这时候反而可以回过头来认证下握手过程**。
