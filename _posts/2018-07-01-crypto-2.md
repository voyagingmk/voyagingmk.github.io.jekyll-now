...
问题：攻击者还是截包，就获得了secret，因为每次登录都是发一样的secret，所以攻击者首先可以做**重放攻击**，直接copy用户的登录包，发给服务器，就可以登录了；其次，因为Hash函数无非md5、SHA1、SHA256这些，攻击者可以用raintable暴力查表，很可能可以破解出password。
