...
PRF的简单实现是hash(msg || key)，这是非常naive的，会遇到[Length extension attack](https://en.wikipedia.org/wiki/Length_extension_attack)。严格的PRF实现是HKDF，下篇文章再介绍。
...
mmtls是对TLS1.3草案的精简，觉得与其分析mmtls还不如先好好看一遍TLS1.3草案。
