...
pool = usedpools[3 + 3] = usedpools[2*3] = &usedpools[3]


&usedpools[sizeClassIdx]就是聚合了所有处于used态的、对应这个sizeClassIdx的pools链表的头节点。

**所以这一行操作，就是为了拿到这个used链表的表头。**
...
1. 首先是把新的pool的nextpool和prevpool指针指向了next
2. 修改next的nextpool和prevpool，指向了这个pool
3. next = usedpools[size + size] = &usedpools[size]
4. next的nextpool和prevpool，对应usedpools[size]->nextpool和usedpools[size]->prevpool
5. usedpools[size]->nextpool等于usedpools[size + 1]
6. 和usedpools[size]->prevpool等于usedpools[size + 2]
