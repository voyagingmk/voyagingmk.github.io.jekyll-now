...
这个东西，其实一目了然了，用3个重载保证remove_reference<类型>::type肯定没有&符号。

可以测试一下:



remove_reference在std::forward会被使用。
...
- 以左值a(a具名，所以是左值)作为参数去调用三个实例化模板函数，进入的都是左值Forward
- 以右值10(10不具名，所以是右值)作为参数去调用三个实例化模板函数，进入的都是右值Forward
- 在左值Forward函数体内，p的类型是int&(三次调用都是)
- 在右值Forward函数体内，p的类型是int&&(三次调用都是)
...
- 也就是说，当参数是左值时，它必然是以int&(即左值引用)的形式进入到Forward;
