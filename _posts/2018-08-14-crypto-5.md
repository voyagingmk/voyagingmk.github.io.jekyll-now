...
为什么^0x1b能使得2x不超出GF(256)呢，这又是用到了GF(256)的乘约运算。0x1b是一个不可约多项式的系数，写成二进制就是0b00011011，写成多项式就是：
...
- F(x)最高位为1：\\( (2F(x)表示的多项式) + (x\^{4} + x\^{3} + x + 1) \\)（式1）
- F(x)最高位为0：\\( (2F(x)表示的多项式) \\)（式2）


前面的MixColumns函数也很复杂，用伪代码简化下：

```c
  for (i = 0; i < 4; ++i)
  {  
    t   = a[0];
    Tmp = a[0] ^ a[1] ^ a[2] ^ a[3] ; // Tmp = a[0] + a[1] + a[2] + a[3]
    Tm  = a[0] ^ a[1] ; Tm = xtime(Tm);  a[0] ^= Tm ^ Tmp ; // a[0] = a[0] + (a[0] + a[1])x + Tmp
    Tm  = a[1] ^ a[2] ; Tm = xtime(Tm);  a[1] ^= Tm ^ Tmp ; // a[1] = a[1] + (a[1] + a[2])x + Tmp
    Tm  = a[2] ^ a[3] ; Tm = xtime(Tm);  a[2] ^= Tm ^ Tmp ; // a[2] = a[2] + (a[2] + a[3])x + Tmp
    Tm  = a[3] ^ t ; Tm = xtime(Tm);  a[3] ^= Tm ^ Tmp ; // a[3] = a[3] + (a[2] + a[0])x + Tmp
  }
```
...
a[1] = 2a[1] + 2a[2] + Tmp
a[2] = 2a[2] + 2a[3] + Tmp
a[3] = a[3] + 2a[2] + 2a[0] + Tmp

a[0] = 2a[0] + 2a[1] + a[0] + a[1] + a[2] + a[3]
a[1] = 2a[1] + 2a[2] +  a[0] + a[1] + a[2] + a[3]
a[2] = 2a[2] + 2a[3] + a[0] + a[1] + a[2] + a[3]
a[3] = a[3] + 2a[2] + 2a[0] + a[0] + a[1] + a[2] + a[3]

a[0] = 3a[0] + 3a[1] + a[2] + a[3]
a[1] = 3a[1] + 3a[2] + a[0] + a[3]
a[2] = 3a[2] + 3a[3] + a[0] + a[1]
a[3] = 2a[3] + 3a[2] + 3a[0] + a[1]

a[0] = 3a[0] + 3a[1] +  a[2] + a[3]
a[1] =  a[0] + 3a[1] + 3a[2] + a[3]
a[2] =  a[0] +  a[1] + 3a[2] +3a[3]
a[3] = 3a[0] +  a[1] + 3a[2] +2a[3]
