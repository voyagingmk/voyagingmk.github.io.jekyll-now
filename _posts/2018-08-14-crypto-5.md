...
(x<<1)又是什么含义呢？因为x也是个单字节，也表示多项式的各个系数，x左移1位，相当于系数都左移位，也即所有项都**乘以x**（这就是为什么叫xtime）。

因为在GF(256)里，异或操作就意味着多项式的同次数项的相加，所以上面的式1和式2其实就是GF的多项式加法：

- x最高位为1：\\( x\cdot P(x) + x\^{4} + x\^{3} + x + 1 \\)（式1）
- x最高位为0：\\( x\cdot P(x) \\)（式2）
