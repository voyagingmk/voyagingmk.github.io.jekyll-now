...
- 8字节，1302 1303 1301 00ff，4个cipher_suites
- 1字节，01，未知，可能是分隔符
...
接下来看tcpdump.out，发现服务器收到ClientHello后发了个很长的回包，足足有1451（不包含IP header）。看起来很长， 实则是几个协议包被tcp合并发送了。先看第一个协议包。
...
最后2字节是007a，十进制是122，表示正文是122字节。


正文结构和ClientHello很像，除了几点要说的：

- ClientHello的Version本来是TLSv1.0，这是为了应付低版本的服务器，实际上在client_version里指出自己支持TLS 1.2。ServerHello于是把Version填成TLS1.2，以告诉客户端自己支持1.2。
...
### ChangeCipherSpec（6字节）

头5字节：14 03 03 00 01

注意第一个字节从16变成了14，代表ChangeCipher。

ChangeCipherSpec（6字节）不多说，在TLS1.3中已被移除。
