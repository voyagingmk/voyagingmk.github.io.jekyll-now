...
- zltail：表示这个ziplist头和尾之间的偏移量，即最后一个字节和zl首地址的差，初始偏移量等于头部大小（10字节）
...
- unsigned char *ziplistPush(unsigned char *zl, unsigned char *s, unsigned int slen, int where); 插入头或尾，实际是调用Insert
- unsigned char *ziplistInsert(unsigned char *zl, unsigned char *p, unsigned char *s, unsigned int slen); 调用Insert
