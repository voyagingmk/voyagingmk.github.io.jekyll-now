...
可以这样思考一下，假设我偏偏不找投影面积最大的平面去投影，我找最小的，那会如何呢？比如说在三视图里挑了一个投影面积只有1个像素的平面去光栅化！**那么光栅化产生的fragment也就只有一个**，这一个fragment只会让fragment shader运行一次，也就是产生一个体素，这个体素代表了整个三角形！

假设在另一个视图下这个三角形其实面积很大，可能产生了100x100个fragment，那么这个错误的投影就导致了体素的严重退化，从10000个体素退化成单一个体素，精度直线下降。
